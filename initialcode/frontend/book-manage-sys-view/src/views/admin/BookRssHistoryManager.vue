...
<template>
    <el-row style="background-color: #FFFFFF;padding: 20px 0;border-radius: 5px;">
        <el-row style="padding: 10px;margin: 0 10px;">
            <!-- 上方的搜索框及新增框 -->
            <el-row>
                <<span class="top-bar">通知時間查詢</span>
        ...
        <el-row style="margin: 10px 20px;">
            <el-table row-key="id" @selection-change="handleSelectionChange" :data="tableData" style="width: 100%">
                ...
                <el-table-column prop="bookName" width="128" label="書籍"></el-table-column>
                <el-table-column prop="userName" width="128" label="訂閱者"></el-table-column>
                ...
                <el-table-column prop="bookshelfId" width="148" label="書架">
                    <template slot-scope="scope">
                        <samp>{{ scope.row.shelfFloor + "樓-" + scope.row.shelfArea + "-" + scope.row.shelfName }}</samp>
                    </template>
                </el-table-column>
    ...
    methods: {
        ...
        async fetchFreshData() {
            try {
                ...
                const response = await this.$axios.post('/bookRssHistory/query', params); //把params也發送到後端
                const { data } = response;
