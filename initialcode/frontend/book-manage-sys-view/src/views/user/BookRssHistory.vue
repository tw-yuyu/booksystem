    ...
    <el-row style="background-color: #FFFFFF;padding: 20px 0;border-radius: 5px;">
        ...
        <el-row style="margin: 10px 20px;">
            <el-table row-key="id" :data="tableData" style="width: 100%">
                ...
                <el-table-column prop="createTime" width="148" label="訂閱時間"></el-table-column>
                <el-table-column label="操作" fixed="right" width="150">
                    <template slot-scope="scope">
                        <!--  scope.row 代表當前行的數據（對應 tableData 中的每一個元素） 可以靈活運用  scope.row.name就是當前行的name屬性  -->
                        <span class="text-button" @click="handleDelete(scope.row)">取消訂閱</span>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
    data() {
        return {
            ...
            bookRssHistoryQueryDto: {}, // 搜尋條件 傳進Query後端
            searchTime: []//儲存範圍時間 
        ...
        async fetchFreshData() {
            try {
                ...
                const params = {
                    ...
                    size: this.pageSize,
                    ...this.bookRssHistoryQueryDto, //將模糊搜尋裡的字串傳進來並放在data() 在這裡讀取傳給後端 什麼都沒傳就查詢全部mapper  {current: 1, size: 7, startTime: null, endTime: null}
                    startTime: startTime,//增加查詢開始時間
                    endTime: endTime,//增加查詢結束時間
                };
                const response = await this.$axios.post('/bookRssHistory/queryUser', params); //把params也發送到後端
