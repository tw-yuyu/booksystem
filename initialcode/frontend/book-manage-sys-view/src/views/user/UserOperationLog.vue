    ...
    <el-row style="background-color: #FFFFFF;padding: 20px 0;border-radius: 5px;">
        ...
        <el-row style="margin: 10px 20px;">
            <el-table row-key="id" :data="tableData" style="width: 100%">
                ...
                <el-table-column prop="content" label="行為"></el-table-column>
                <el-table-column prop="createTime" width="148" label="行為時間"></el-table-column>
...
export default {
    data() {
        return {
            ...
            userOperationLogQueryDto: {}, // 搜尋條件 傳進Query後端
            searchTime: []//儲存範圍時間 
        ...
        async fetchFreshData() {
            try {
                ...
                const params = {
                    ...
                    size: this.pageSize,
                    ...this.userOperationLogQueryDto, //將模糊搜尋裡的字串傳進來並放在data() 在這裡讀取傳給後端 什麼都沒傳就查詢全部mapper  {current: 1, size: 7, startTime: null, endTime: null}
                    startTime: startTime,//增加查詢開始時間
                    endTime: endTime,//增加查詢結束時間
                };
                const response = await this.$axios.post('/userOperationLog/queryUser', params); //把params也發送到後端
